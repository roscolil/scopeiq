  ...
  const searchSuppliers = () => {
    ...
    setTimeout(() => {
      ...
      const mockSuppliers: Supplier[] = [
        {
          ...
          lng: userLocation ? userLocation.longitude - 0.02 : -74.0060,
        },
        {
          id: "sup2",
          ...
          lng: userLocation ? userLocation.longitude + 0.01 : -74.0260,
        },
        {
          id: "sup3",
          ...
          lng: userLocation ? userLocation.longitude + 0.03 : -73.9860,
        },
      ];
      
      // Filter suppliers based on the search query
      const filteredSuppliers = mockSuppliers.filter(supplier => {
        const matchesName = supplier.name.toLowerCase().includes(query.toLowerCase());
      ...
      if (userLocation) {
        ...
        // Sort by distance
        filteredSuppliers.sort((a, b) => (a.distance || Infinity) - (b.distance || Infinity));
      }
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          ...
          {userLocation ? (
            <p className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
              <MapPin className="h-3 w-3" /> Location available - search will show distances
            </p>
          ) : (
            <p className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
              <MapPin className="h-3 w-3" /> Enable location for distance information
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {suppliers.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold">Results ({suppliers.length})</h2>
                ...
                <CardContent className="pt-6">
                  <div className="flex justify-between">
                    ...
                    {supplier.distance !== undefined && (
                      <div className="bg-muted text-muted-foreground px-2 py-1 rounded-md text-sm flex items-center">
                        <MapPin className="h-3 w-3 mr-1" /> {supplier.distance} km
                      </div>
                    )}
                  </div>
