 ...
 * Reliable, managed vector database with free tier
 */
...
// Cache for client and index reference
let pineconeClient: Pinecone | null = null
let indexCache: ReturnType<Pinecone['index']> | null = null

// Helper: Get or initialize Pinecone client
function getPineconeClient() {
  ...
  if (!apiKey) {
    throw new Error(
      'VITE_PINECONE_API_KEY not found. Please add your Pinecone API key to the .env file.',
    )
  }

  if (!apiKey.startsWith('pcsk_')) {
    throw new Error(
      'Invalid Pinecone API key format. API key should start with "pcsk_"',
    )
  }
...
async function getOrCreateIndex() {
  ...
  try {
    ...
    if (!existingIndex) {
      ...
      await new Promise(resolve => setTimeout(resolve, 60000)) // Wait 60 seconds for serverless
    }
