...
export function formatChatForSharing(
  ...
  const header = `🔍 Jack Chat Conversation
...
Generated by Jack AI Assistant
...
export function downloadChatConversation(
  ...
  const filename = `jack-chat-${new Date().toISOString().split('T')[0]}.txt`
...
export function formatChatAsHTML(
  ...
  const footer = `
    <div class="footer">
      Generated by Jack AI Assistant<br>
  ...
  return `
    ...
    <head>
      ...
      <title>Jack Chat Conversation - ${date}</title>
      ${styles}
    </head>
    <body>
      ${header}
...
export function downloadChatAsHTML(
  ...
  const filename = `jack-chat-${new Date().toISOString().split('T')[0]}.html`
