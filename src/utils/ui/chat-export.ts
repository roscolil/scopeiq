...
export function formatChatForSharing(
  ...
  const header = `🔍 Jacq Chat Conversation
...
Generated by Jacq AI Assistant
...
export function downloadChatConversation(
  ...
  const filename = `jacq-chat-${new Date().toISOString().split('T')[0]}.txt`
...
export function formatChatAsHTML(
  ...
  const footer = `
    <div class="footer">
      Generated by Jacq AI Assistant<br>
  ...
  return `
    ...
    <head>
      ...
      <title>Jacq Chat Conversation - ${date}</title>
      ${styles}
    </head>
    <body>
      ${header}
...
export function downloadChatAsHTML(
  ...
  const filename = `jacq-chat-${new Date().toISOString().split('T')[0]}.html`
