...
/**
 * HOC for protecting entire components with authorization
 */
export function withAuthorization<P extends object>(
  ...
  return function AuthorizedComponent(props: P) {
    ...
    if (authConfig.requireRole) {
      const roles = Array.isArray(authConfig.requireRole)
        ? authConfig.requireRole
        : [authConfig.requireRole]
      if (!hasAnyRole(roles)) {
        return (authConfig.fallback as React.ReactElement) || null
      }
    }

    // Check permission requirements
    if (authConfig.requirePermission) {
      const permissions = Array.isArray(authConfig.requirePermission)
        ? authConfig.requirePermission
        : [authConfig.requirePermission]

      const hasRequiredPermissions = authConfig.requireAnyPermission
    ...
    if (
      authConfig.requireProject &&
      !canAccessProject(authConfig.requireProject)
    ) {
      return (authConfig.fallback as React.ReactElement) || null
    }
